...
class TicketLifecycleAggregationService {
  ...
  async getAggregationData(params) {
    ...
    if (grouping === 'day') {
      ...
      `;
      queryParams = status ? [startDate, endDate, status] : [startDate, endDate];
    } else {
      query = `
        SELECT 
          bucket_iso_year,
      ...
      `;
      queryParams = status ? [startDate, endDate, status, this.timezone] : [startDate, endDate, this.timezone];
    }
